...
public class ActionLogAPIController {
	...
	JqPageObject getLogList(ActionLog systemLog) {
		...
		int totalRecords = redisTemplate.opsForList().size(CommonConstants.KEY_SYSTEM_LOG_ACTION).intValue(); //total count of records
		...
		List<String> logJsonList = redisTemplate.opsForList()
				.range(CommonConstants.KEY_SYSTEM_LOG_ACTION, start, end);
